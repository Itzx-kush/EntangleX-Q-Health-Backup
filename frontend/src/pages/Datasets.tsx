...
export default function Datasets() {
  ...
  const [file, setFile] = useState<File | null>(null); const [deidentified, setDeidentified] = useState(false); const [uploadSizeError, setUploadSizeError] = useState('');
  ...
  async function upload(e: FormEvent) {
    e.preventDefault(); setUploadSizeError(''); if (!file || !deidentified) return;
    if (file.size > 4 * 1024 * 1024) {setUploadSizeError('CSV uploads are limited to 4 MB on the hosted demo.'); return;}
  ...
  return <><PageHeader eyebrow="01 / Dataset provenance" title="Know the source." description="Register a public benchmark or a deidentified CSV. Dataset bytes are hashed and remain immutable once referenced by an experiment."><button disabled={action.busy} onClick={() => void demo()}>Load Wisconsin benchmark</button></PageHeader><ErrorNotice message={action.error || uploadSizeError || list.error}/>
    ...
    <div className="two-columns"><Card title="Upload a deidentified CSV"><form onSubmit={e => void upload(e)}><div className="form-grid">{Object.entries(form).map(([key, value]) => <Field key={key} label={key.replaceAll('_', ' ')}><input required={!['source_url'].includes(key)} maxLength={key === 'source_url' ? 500 : 160} value={value} onChange={e => setForm({...form, [key]: e.target.value})}/></Field>)}</div><Field label="CSV file" help="UTF-8, unique headers, one target with exactly two classes; hosted demo upload limit 4 MB."><input type="file" accept=".csv,text/csv" required onChange={e => {setUploadSizeError(''); setFile(e.target.files?.[0] || null);}}/></Field><label className="check"><input type="checkbox" required checked={deidentified} onChange={e => setDeidentified(e.target.checked)}/>I confirm this file is suitable for local research, contains no patient-identifiable information, and represents independent samples.</label><button type="submit" disabled={action.busy || !file || !deidentified}>{action.busy ? 'Processing...' : 'Validate and register dataset'}</button></form></Card>
